...
def login():
    ...
    if form.validate_on_submit():
        try:
            user = User.query.filter(User.user_name == form.name.data).all()[0]
        except Exception:
            user = None
        if user and check_password_hash(user.pwd, form.pwd.data):
            rm = form.remember.data
            login_user(user, remember=rm)
            return redirect(request.args.get("next") or url_for("entity.index"))
...
def registration():
    ...
    form = RegistrationForm(lst=users)
    if form.validate_on_submit():
        try:
            hash_pwd = generate_password_hash(form.pwd.data)
            user = User(user_name=form.name.data, pwd=hash_pwd)
            db.session.add(user)
            db.session.commit()
    ...
    return render_template('registration.html', form=form, lst=users)
